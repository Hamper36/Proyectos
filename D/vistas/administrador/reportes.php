...
// 2. Usuarios por Rol
$res_u = $conexion->query("SELECT r.nombre_rol, COUNT(u.id_usuario) AS cantidad 
                                  FROM rol r 
...
// 3. Vehículos por estado (usando estado_vehiculo, la fuente confiable)
$res_v = $conexion->query("SELECT estado_vehiculo AS estado, COUNT(*) AS cantidad FROM vehiculo GROUP BY estado_vehiculo");
$vehiculos_stats = ['Apto' => 0, 'Pendiente' => 0, 'No Apto' => 0, 'Inactivo' => 0];
...
if ($tipo == 'Usuarios') {
    ...
    $stmt_det->execute();
    $reporte_detallado = $stmt_det->get_result()->fetch_all(MYSQLI_ASSOC);
} elseif ($tipo == 'Vehículos') {
    $res_det = $conexion->query("SELECT v.*, u.nombres, u.apellidos 
                                       FROM vehiculo v 
                                       JOIN chofer ch ON ch.id_chofer = v.id_chofer 
                                       JOIN usuario u ON u.id_usuario = ch.id_usuario 
                                       ORDER BY v.id_vehiculo DESC");
    $reporte_detallado = $res_det->fetch_all(MYSQLI_ASSOC);
} elseif ($tipo == 'Pagos') {
                                            ...
                                            <tbody>
                                                <?php foreach ($reporte_detallado as $item) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php
                                                            $badge_v = $item['estado_vehiculo'] == 'Apto' ? 'bg-success' : ($item['estado_vehiculo'] == 'No Apto' ? 'bg-danger' : 'bg-warning text-dark');
                                                            ?>
                                                            <span class="badge <?php echo $badge_v; ?>"><?php echo htmlspecialchars($item['estado_vehiculo']); ?></span>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
                                            ...
                                            <tbody>
                                                <?php foreach ($reporte_detallado as $item) { ?>
                                                    <tr>
                                                        ...
                                                        <td><span class="badge bg-secondary font-monospace"><?php echo htmlspecialchars($item['numero_referencia']); ?></span></td>
